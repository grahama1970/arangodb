...
try:
    ...
    from ..db_operations import create_relationship, delete_relationship_by_key
except ImportError:
    try:
        # Try absolute import for package structure
        from arangodb.core.db_operations import create_relationship, delete_relationship_by_key
    except ImportError:
        # Fallback to providing dummy functions if not available
        logger.warning("Could not import db_operations, using fallbacks")
        
        def create_relationship(db, _from, _to, relationship_type, **attributes):
            """Fallback create_relationship function."""
            doc = {
                "_from": _from,
                "_to": _to,
            ...
            # If edge collection is defined in attributes
            edge_collection = attributes.get("edge_collection", "relationships")
            return {"_key": "test_key", "_id": f"{edge_collection}/test_key"}
            
        def delete_relationship_by_key(db, key, edge_collection="relationships"):
            """Fallback delete_relationship_by_key function."""
            return True
...
def invalidate_edge(
    ...
    try:
        ...
        current_doc = edge_coll.get(edge_key)
        
        logger.debug(f"Current doc before update: {current_doc}")
        logger.debug(f"Update data: {update_data}")
        
        # Update the document with new fields
        current_doc.update(update_data)
        
        # Make sure we have required edge fields
        if "_from" not in current_doc or "_to" not in current_doc:
            logger.error(f"Document missing required fields: _from or _to")
            return None
        
        # Remove system fields but keep required edge fields
        clean_doc = {k: v for k, v in current_doc.items() if not k.startswith("_")}
        
        # Ensure required edge fields are included
        clean_doc["_from"] = current_doc["_from"]
        clean_doc["_to"] = current_doc["_to"]
        
        logger.debug(f"Clean doc for replacement: {clean_doc}")
        
        # Replace the entire document (add _key to the doc)
        clean_doc["_key"] = edge_key
        result = edge_coll.replace(clean_doc)
        
        logger.debug(f"Replace result: {result}")
        logger.info(f"Invalidated edge {edge_key} in {edge_collection}")
        
        # Fetch the document to ensure we have the latest version
        updated_doc = edge_coll.get(edge_key)
        logger.debug(f"Updated document after replace: invalid_at={updated_doc.get('invalid_at')}")
        return updated_doc
...
def create_temporal_relationship(
    ...
    try:
        ...
        if check_contradictions and contradictions and created_edge:
            for contradiction in contradictions:
                update_result = db.collection(edge_collection).update(
                    {"_key": contradiction["_key"]},
                    {"invalidated_by": created_edge["_key"]}
                )
                logger.debug(f"Updated invalidated_by for contradiction {contradiction['_key']}: {update_result}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        database_name = os.getenv("ARANGO_DB_NAME", "memory_bank")
        ...
        # Debug logging
        logger.info(f"All edges found: {len(all_edges)}")
        for i, edge in enumerate(all_edges):
            logger.info(f"Edge {i}: _key={edge.get('_key')}, invalid_at={edge.get('invalid_at')}, invalidated_by={edge.get('invalidated_by')}")
