...
Also supports cross-encoder reranking for improved result relevance.
...
def clean_json_string(json_str, return_dict=False):
    """Mock function for testing"""
    if return_dict:
        return {} if not json_str else json.loads(json_str)
    return json_str
...
try:
    ...
    from arangodb.search_api.tag_search import tag_search
    from arangodb.search_api.graph_traverse import graph_search
    from arangodb.search_api.cross_encoder_reranking import rerank_search_results, cross_encoder_rerank
    HAS_CROSS_ENCODER_RERANKING = True
except ImportError:
    logger.warning("Could not import all search modules, using mocks for testing")
    ...
    def graph_search(*args, **kwargs):
        """Mock function for testing"""
        return {"results": [], "total": 0}
        
    # Cross-encoder reranking mock
    def cross_encoder_rerank(*args, **kwargs):
        """Mock function for testing"""
        return []
        
    def rerank_search_results(search_results, **kwargs):
        """Mock function for testing"""
        search_results["reranking_applied"] = False
        search_results["reranking_error"] = "Mock implementation"
...
def store_perplexity_results(
    ...
    try:
        ...
        search_results["stored_perplexity"] = {
            ...
            "edges": created_edges,
            "topics": [topic.model_dump() for topic in perplexity_response.topics],
...
def hybrid_search(
    ...
    Can further improve results using cross-encoder reranking.
    ...
    Args:
        ...
        use_reranking: Whether to apply cross-encoder reranking for improved results
        reranking_model: Cross-encoder model to use for reranking
        reranking_strategy: Strategy for combining original and reranked scores
                           ("replace", "weighted", "max", "min")
        reranking_weights: Weights for combining scores (when strategy is "weighted")
    ...
    # Default reranking weights if not provided
    if reranking_strategy == "weighted" and not reranking_weights:
        reranking_weights = {
            "original": 0.3,
            "cross_encoder": 0.7
        }
    ...
    try:
        ...
        # STEP 6: Apply cross-encoder reranking if requested
        reranking_time = 0
        if use_reranking and HAS_CROSS_ENCODER_RERANKING:
            ...
            # Create search results structure for reranking
            pre_rerank_results = {
                "query": query_text,
                "results": final_results[:top_n * 2],  # Rerank more than needed
                "search_engine": "hybrid" + ("-graph" if use_graph else "")
            }
            
            # Apply reranking
            reranked_results = rerank_search_results(
                search_results=pre_rerank_results,
                model_name=reranking_model,
                score_field="hybrid_score",
                top_k=top_n,
                score_combination_strategy=reranking_strategy,
                score_combination_weights=reranking_weights
            )
            
            if reranked_results.get("reranking_applied", False):
                # Use reranked results
                final_results = reranked_results.get("results", [])
                logger.info(f"Reranking applied successfully, got {len(final_results)} results")
                reranking_time = time.time() - reranking_time_start
            else:
                # Reranking failed, log and continue with original results
                logger.warning(f"Reranking failed: {reranked_results.get('reranking_error', 'Unknown error')}")
        ...
        # Add reranking info if used
        if use_reranking and HAS_CROSS_ENCODER_RERANKING:
            response["reranking_time"] = reranking_time
            if reranking_time > 0:
                response["search_engine"] += "+reranked"
                response["reranking_params"] = {
                    "model": reranking_model,
                    "strategy": reranking_strategy
                }
                if reranking_strategy == "weighted":
                    response["reranking_params"]["weights"] = reranking_weights
...
def print_hybrid_search_results(search_results: Dict[str, Any], max_width: int = 120) -> None:
    ...
    if output_format == "json":
        ...
        if "reranking_time" in search_results:
            json_results["reranking_time"] = search_results.get("reranking_time", 0)
            json_results["reranking_params"] = search_results.get("reranking_params", {})
    ...
    if "reranking_time" in search_results and search_results.get("reranking_time", 0) > 0:
        reranking_params = search_results.get("reranking_params", {})
        model_name = reranking_params.get("model", "unknown")
        strategy = reranking_params.get("strategy", "replace")
        
        print(f"Reranking: Model={Fore.MAGENTA}{model_name.split('/')[-1]}{Style.RESET_ALL}, " +
              f"Strategy={Fore.MAGENTA}{strategy}{Style.RESET_ALL}, " +
              f"Time={Fore.CYAN}{search_results.get('reranking_time', 0):.3f}s{Style.RESET_ALL}")
...
def print_result_details(result: Dict[str, Any]) -> None:
    ...
    cross_encoder_score = result.get("cross_encoder_score", None)
    final_score = result.get("final_score", hybrid_score)
    ...
    for field in display_fields:
        if field in doc and doc[field]:
            ...
            safe_value = truncate_large_value(doc[field], max_length=100)
    ...
    if cross_encoder_score is not None:
        if cross_encoder_score > 0.9:
            ce_str = f"{Fore.GREEN}{cross_encoder_score:.2f}{Style.RESET_ALL}"
        elif cross_encoder_score > 0.7:
            ce_str = f"{Fore.YELLOW}{cross_encoder_score:.2f}{Style.RESET_ALL}"
        else:
            ce_str = f"{Fore.WHITE}{cross_encoder_score:.2f}{Style.RESET_ALL}"
        print(f"  • {Fore.MAGENTA}Cross-Encoder Score:{Style.RESET_ALL} {ce_str}")
    
    # Final score (if different from hybrid)
    if final_score != hybrid_score:
        if final_score > 0.9:
            final_str = f"{Fore.GREEN}{final_score:.2f}{Style.RESET_ALL}"
        elif final_score > 0.7:
            final_str = f"{Fore.YELLOW}{final_score:.2f}{Style.RESET_ALL}"
        else:
            final_str = f"{Fore.WHITE}{final_score:.2f}{Style.RESET_ALL}"
        print(f"  • {Fore.MAGENTA}Final Score:{Style.RESET_ALL} {final_str}")
...
def validate_hybrid_search(search_results: Dict[str, Any], fixture_path: str) -> Tuple[bool, Dict[str, Dict[str, Any]]]:
    ...
    # Skip reranking validation if it's not consistently applied
    if "reranking_applied" in search_results and "validate_reranking" not in expected_data:
        logger.info("Skipping validation of reranking results as they may vary each time")
...
if __name__ == "__main__":
    ...
    for i, arg in enumerate(sys.argv):
        if arg == "--format" and i+1 < len(sys.argv):
            ...
            use_reranking = True
        elif arg == "--rerank-model" and i+1 < len(sys.argv):
            reranking_model = sys.argv[i+1]
            use_reranking = True
        elif arg == "--rerank-strategy" and i+1 < len(sys.argv):
            reranking_strategy = sys.argv[i+1]
            use_reranking = True
    ...
    try:
        ...
        # Skip validation if using perplexity or reranking as they will produce different results each time
        if not use_perplexity and not use_reranking:
            ...
            logger.info("Skipping validation for search with Perplexity or reranking integration")
