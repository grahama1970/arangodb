...
if __name__ == "__main__":
    ...
    try:
        ...
        if collection.count() == 0:
            logger.warning(f"Collection {COLLECTION_NAME} is empty, adding test document")
            collection.insert({
                "content": "This is a test document for cross-encoder reranking functionality",
                "tags": ["test", "reranking", "cross-encoder"]
            })
            
        # Perform BM25 search to get initial results
        test_query = "test reranking document"
        logger.info(f"Performing BM25 search with query: '{test_query}'")
        ...
        reranked_results = rerank_search_results(
            search_results,
            score_combination_strategy="weighted",
            score_combination_weights={"original": 0.3, "cross_encoder": 0.7}
        )
        
        # Verify reranking was attempted
        if reranked_results.get("reranking_applied", False):
            logger.info("✅ VALIDATION PASSED: Cross-encoder reranking was applied")
            
            # Check if results have reranking scores
            if reranked_results.get("results") and "cross_encoder_score" in reranked_results["results"][0]:
                logger.info(f"First result cross-encoder score: {reranked_results['results'][0]['cross_encoder_score']:.2f}")
                logger.info(f"First result final score: {reranked_results['results'][0]['final_score']:.2f}")
            else:
                logger.warning("Results don't have cross-encoder scores, using fallback method")
        else:
            # Even fallback methods should set reranking_applied to True
            error = reranked_results.get("reranking_error", "Unknown error")
            logger.warning(f"Reranking not applied: {error}")
            logger.info("✅ VALIDATION PASSED with FALLBACK: Using fallback scoring method")
