...
def resolve_contradiction(
    ...
    try:
        ...
        if not validate_temporal_metadata(new_edge)[0]:
            return {
                "action": "error",
        ...
        if not validate_temporal_metadata(contradicting_edge)[0]:
            return {
                "action": "error",
        ...
        # Strategy: Merge temporal ranges
            ...
            if new_key:
                result = db.collection(edge_collection).update(
                    new_key,
                    update_data,
                    return_new=True
                )
                ...
                if result and "new" in result:
                    resolved_edge = result["new"]
                    success = True
                else:
                    # If the new edge doesn't exist yet, prepare it for creation
