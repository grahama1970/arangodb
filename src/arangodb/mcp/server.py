"""
arangodb FastMCP Server
Module: server.py
Description: Functions for server operations

Granger standard MCP server implementation for arangodb.
"""

from fastmcp import FastMCP
from .arangodb_prompts import register_all_prompts
from .prompts import get_prompt_registry

# Initialize server
mcp = FastMCP("arangodb")
mcp.description = "arangodb - Granger spoke module"
...
async def capabilities() -> str:
    ...
    return await prompt_registry.execute("arangodb:capabilities")


@mcp.prompt()
async def help(context: str = None) -> str:
    """Get context-aware help"""
    return await prompt_registry.execute("arangodb:help", context=context)


@mcp.prompt()
async def quick_start() -> str:
    """Quick start guide for new users"""
    return await prompt_registry.execute("arangodb:quick-start")
...
if __name__ == "__main__":
    ...
    async def validate():
        ...
        assert "arangodb" in result.lower()
