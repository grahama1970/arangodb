"""
Workflow logging module for gitget integration.

This module provides detailed logging for complex gitget workflows, integrating
the enhanced logger with the error handling system.

Sample Input:
    ```python
    from gitget.utils.workflow_logger import WorkflowLogger, track_workflow, track_step
    
    @track_workflow("repository_processing")
    def process_repository(repo_url, output_dir, **kwargs):
        # Get workflow logger injected by decorator
        workflow_logger = kwargs.get('workflow_logger')
        workflow_logger.log_info("Starting repository processing")
        
        # Process steps
        step1_result = extract_metadata(repo_url, workflow_logger=workflow_logger)
        step2_result = clone_repository(repo_url, output_dir, workflow_logger=workflow_logger)
        
        # Log completion
        workflow_logger.log_completion(success=True, result={
            "files_processed": 45,
            "total_size": 1250000
        })
        return {"status": "success", "files": 45}
    
    @track_step("metadata_extraction")
    def extract_metadata(repo_url, workflow_logger=None):
        workflow_logger.log_info(f"Extracting metadata from {repo_url}")
        # Extraction logic...
        return {"repository_info": {"owner": "example", "stars": 120}}
    ```

Expected Output:
    ```
    [2025-05-06 14:30:45.123] [INFO] [WorkflowLogger] [repository_processing] Workflow started
    [2025-05-06 14:30:45.130] [INFO] [WorkflowLogger] [repository_processing] Starting repository processing
    [2025-05-06 14:30:45.242] [INFO] [WorkflowLogger] [repository_processing:metadata_extraction] Step started
    [2025-05-06 14:30:45.250] [INFO] [WorkflowLogger] [repository_processing:metadata_extraction] Extracting metadata from https://github.com/example/repo
    [2025-05-06 14:30:46.350] [INFO] [WorkflowLogger] [repository_processing:metadata_extraction] Step completed (1.108s)
    [2025-05-06 14:30:48.720] [INFO] [WorkflowLogger] [repository_processing] Workflow completed successfully (3.597s)
    [2025-05-06 14:30:48.725] [INFO] [WorkflowLogger] [repository_processing] Result metrics: files_processed=45, total_size=1250000
    ```
"""
...
from gitget.utils.error_handler import (
    ErrorHandler, ErrorSource, ErrorSeverity, GitGetError, global_error_handler
)
...
class WorkflowLogger:
    """
    Logger for complex gitget workflows.
    ...
    def log_error(
        self,
        error: Union[Exception, GitGetError],
        ...
        # Handle the error
        if isinstance(error, GitGetError):
            # If it's already a GitGetError, just handle it
            self.errors.append(error)
            return self.error_handler.handle_error(error)
        else:
            # If it's an exception, create a GitGetError and handle it
...
def track_workflow(workflow_name: str):
    """
    Decorator for tracking entire workflows.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            workflow_logger = WorkflowLogger(workflow_name)
            
            try:
                # Add workflow logger to kwargs if the function accepts it
                if 'workflow_logger' in kwargs or any(
                    p.kind == p.VAR_KEYWORD for p in inspect.signature(func).parameters.values()
                ):
                    kwargs['workflow_logger'] = workflow_logger
                
                # Execute the function
                result = func(*args, **kwargs)
                
                # Log completion
                workflow_logger.log_completion(success=True, result=result)
                
                return result
            except Exception as e:
                # Log error and completion
                workflow_logger.log_error(e)
                workflow_logger.log_completion(success=False)
                raise
...
def track_step(
    ...
    """
    Decorator for tracking individual workflow steps.
    
    Args:
        step_name: Name of the step
        component: Component type
        error_source: Error source for potential errors
    ...
    def decorator(func: F) -> F:
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            # Try to find a workflow logger in the arguments
            workflow_logger = None
            for arg in args:
                if isinstance(arg, WorkflowLogger):
                    workflow_logger = arg
                    break
            
            if workflow_logger is None:
                for key, value in kwargs.items():
                    if isinstance(value, WorkflowLogger):
                        workflow_logger = value
                        break
            
            # If no workflow logger found, use a temporary one
            if workflow_logger is None:
                workflow_logger = WorkflowLogger(f"{func.__module__}.{func.__name__}")
            
            # Log step start
            workflow_logger.log_step(step_name, component=component)
            ...
            try:
                ...
                # Log step result
                workflow_logger.log_result(result, step_name=step_name)
