...
class RelationshipExtractor:
    ...
    def extract_relationships_from_text(
        ...
        if "REFERENCES" in relationship_types:
            reference_patterns = [
                ...
                r'[aA]ccording\s+to\s+(?:the\s+)?([\'"]?[\w\s]+[\w][\'"]?)',
            ...
            for pattern in reference_patterns:
                ...
                match_count = sum(1 for _ in re.finditer(pattern, text, re.IGNORECASE))
                logger.debug(f"Pattern '{pattern[:30]}...' found {match_count} matches")
    ...
    def infer_relationships_by_similarity(
        ...
        # First, try using APPROX_NEAR_COSINE for vector search
        aql = f"""
        ...
        try:
            ...
            FILTER doc._id != @doc_id
            FILTER doc.{embedding_field} != null
            FILTER LENGTH(doc.{embedding_field}) == LENGTH(@embedding)
            LET dot_product = SUM(
                FOR i IN 0..LENGTH(doc.{embedding_field})-1
                RETURN doc.{embedding_field}[i] * @embedding[i]
            )
            LET doc_magnitude = SQRT(SUM(
                FOR val IN doc.{embedding_field}
                RETURN val * val
            ))
            ...
            LET score = (doc_magnitude * query_magnitude) > 0 
    ...
    def create_document_relationship(
        ...
        contradicting_edges = find_contradicting_edges(
            self.db,
        ...
        if contradicting_edges:
            # Log the contradiction for now - enhanced_relationships doesn't have resolve function
            logger.warning(f"Found {len(contradicting_edges)} contradicting edges for new relationship")
            # In a production system, you would handle contradictions here
...
if __name__ == "__main__":
    ...
    try:
        # Get authentication from constants
        import os
        os.environ.get("ARANGO_PASSWORD", "openSesame")
        password = "openSesame"  # Default password from constants
    ...
    logger.info(f"Test text length: {len(test_text)}, stripped: {len(test_text.strip())}")
    # Create a source document for context
    source_doc = {"name": "Test Document", "_id": "test_doc_1"}
