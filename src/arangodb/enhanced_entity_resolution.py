...
try:
    from arangodb.utils.embedding_utils import get_embedding, calculate_cosine_similarity
except ImportError:
    from src.arangodb.utils.embedding_utils import get_embedding, calculate_cosine_similarity
...
def normalize_name(name: str) -> str:
    ...
    # Remove punctuation that might vary in names
...
def find_similar_entity_matches(
    ...
    try:
        # Use AQL to find similar entities
        aql = """
        FOR doc IN @@collection
...
def resolve_entity(
    ...
    try:
        ...
        if best_match and best_match.get("_confidence", 0) >= min_confidence and auto_merge:
            ...
            db.collection(collection_name).update(best_match["_key"], merged_entity)
...
if __name__ == "__main__":
    ...
    # Configure ArangoDB connection parameters
    ...
    try:
        ...
        unique_entity = {
            ...
            "embedding": [0.9, 0.8, 0.7, 0.6, 0.5]  # Unique embedding
        }
